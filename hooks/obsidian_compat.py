...
이 hook은 빌드 시 마크다운을 받아 다음만 보정한다(원본 파일은 변경하지 않음):
  ...
  5. Obsidian이 허용하는 `\color{#RRGGBB}`를 MathJax 표준 color 확장이
     지원하는 `\color[RGB]{R,G,B}`로 변환.
...
_HEX_COLOR = re.compile(r"\\color\{#([0-9A-Fa-f]{6})\}")


def _normalize_hex_colors(line):
    """Obsidian의 16진수 색상을 MathJax 표준 RGB 색상으로 변환한다."""
    def to_rgb(match):
        value = match.group(1)
        red, green, blue = (int(value[i : i + 2], 16) for i in (0, 2, 4))
        return rf"\color[RGB]{{{red},{green},{blue}}}"

    return _HEX_COLOR.sub(to_rgb, line)
...
def on_page_markdown(markdown, **kwargs):
    ...
    for i, line in enumerate(lines):
        ...
        line = _normalize_hex_colors(line)
        s = line.strip()
